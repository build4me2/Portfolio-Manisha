    ...
    <article className="group border-b border-dashed border-ink/50 py-6 first:pt-0">
      {hasLink ? (
        <a
          ...
          className="inline-flex items-baseline gap-3 font-display text-base font-bold leading-tight text-ink transition hover:text-inkDark md:text-[1.2rem]"
        >
          {title}
          <span aria-hidden="true" className="font-body text-lg transition group-hover:translate-x-1">→</span>
        </a>
      ) : (
        <span className="font-display text-base font-bold leading-tight text-ink md:text-[1.2rem]">{title}</span>
      )}
